...
public class StdDAOImpl implements StuDAO {
	...
	public int addStudent(Object obj) throws DuplicateException {
		...
		try
		{
			PreparedStatement ps = (PreparedStatement) con.prepareStatement("SELECT * FROM Studentinfo");
		...
		{
			PreparedStatement ps1= (PreparedStatement)con.prepareStatement("Insert into Studentinfo values (?,?,?)");
	...
	public List<Student> getStudent() {
		...
		try
		{
			PreparedStatement ps = (PreparedStatement) db.getConnection().prepareStatement("SELECT * FROM Studentinfo");
			ResultSet rs=ps.executeQuery();
